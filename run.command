...
# A Terminal opened by double-click does not always have Homebrew on PATH -
# typically when the "eval brew shellenv" line was never added to .zprofile.
# Without this, brew, tesseract and pdftoppm all look missing when they are
# not, and the message below wrongly says Homebrew is not installed.
...
# The environment can exist with the packages missing - a first run that lost
# its network halfway, or a requirements.txt that has since grown - and the
# converter would then stop with a traceback. Checking is instant; fix it here.
if ! "$VENV/bin/python" -c 'import pdf2image, pytesseract, cv2, numpy, PIL, docx' >/dev/null 2>&1; then
